...
if __name__ == '__main__':
    ...
    f_T1 = os.path.join(test_path, subject_name + '_t1.nii.gz')
    f_T2 = os.path.join(test_path, subject_name + '_t2.nii.gz')
    f_T1ce = os.path.join(test_path, subject_name + 't1ce.nii.gz')
    f_Flair = os.path.join(test_path, subject_name + 'flair.nii.gz')
    ...
    inputs_T1ce, img_T1ce_itk = read_med_image(f_T1ce, dtype=np.float32)
    inputs_Flair, img_Flair_itk = read_med_image(f_Flair, dtype=np.float32)
    ...
    inputs_T1ce_norm = inputs_T1ce_norm[:, :, :, None]
    inputs_Flair_norm = inputs_Flair_norm[:, :, :, None]
    inputs = np.concatenate((inputs_T1_norm, inputs_T2_norm,inputs_T1ce_norm,inputs_Flair_norm), axis=3)
    inputs = inputs[None, :, :, :, :]
